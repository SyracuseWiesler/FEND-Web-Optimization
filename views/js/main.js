...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    var width = container[0].offsetWidth + determineDx(container[0], size);
    //for (var j = 0; j < container.length; j++){
    //  temp.push(determineDx(container[j], size));
    //}
    for (var i = 0; i < container.length; i++) {
      //var dx = determineDx(container[i], size);
      //var newwidth = (document.querySelectorAll(".randomPizzaContainer")[i].offsetWidth + dx) + 'px';
      container[i].style.width = width + 'px';
      //container[i].style.width = (container[i].offsetWidth + temp) + 'px';
    }
  }
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    items[i].style.left = items[i].basicLeft + 100 * Math.sin(temp + (i % 5)) + 'px';
  }
